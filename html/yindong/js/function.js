	...
	// btn选项卡按钮及父级   
	// con选项卡内容
	function xxka(btn,con,ins){
		for(var i=0;i<btn.length;i++){
		btn[i].ind=i;
		btn[i].onmouseover=function(){
			con[this.ind].style.display="block";
            ins[this.ind].style.display="block";
		};

		btn[i].onmouseout=function(){
			con[this.ind].style.display="none";
            ins[this.ind].style.display="none";
		}
	}
}
...
//初始化第一个轮播点颜色   "#fff"			s2
	...
	function jiedianlunbo(bigBox,imgBox,bannerTu,num,lunboshijian,donghuashijian,zuojian,youjian){
		const box=$(imgBox)[0];
