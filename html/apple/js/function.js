...
// 左右轮播
	...
	// lisColor				轮播点普通颜色			字符串
	// donghuashijian       动画完成的时间			数值
	function zylunbo(pic,bigbannerBox,lunbodian,colorArr,liactivebgColor,lunboTime,donghuaTime,lisColor){	
		const tu=$(pic);
		...
		function move(type="l"){
			...
			img.style.background=color[next];
			}else if(type=="r"){
				next--;
				if(next<0){
					next=tu.length-1;
