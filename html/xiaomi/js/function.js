...
// 左右轮播(没有轮播点)
	function zylunbo2(pic,bigbannerBox,left,right,lunboTime,donghuaTime){	
		const tu=$(pic);
		const img=$(bigbannerBox)[0];
		const imgW=parseInt(window.getComputedStyle(img,null).width);
		const leftbtn=$(left)[0];
...
// 左右轮播 类名
	...
	function zulunbo(lunbotupian,lunbodakuai,lunbodian,zuojian,youjian,lunboTime,donghuaTime,activeClass){
...
//节点左右轮播
	...
	function jiedianlunbo(imgBox,bannerTu,num,lunboshijian,donghuashijian){
		...
		const imgw=parseInt(window.getComputedStyle(img,null).height);
		var flag=true;
		let n=num;
